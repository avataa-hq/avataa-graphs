...
def test_mo_create_not_existing(mo_main_updater, mo_data):
    # check before
    ...
    assert mo_nodes_count_before + 1 == mo_nodes_count_after
    assert mo_edges_count_before + 1 < mo_edges_count_after
    assert mo_path_edges_count_before + 1 <= mo_path_edges_count_after


@pytest.mark.skip(reason="Not implemented")
def test_mo_create_exist(mo_main_updater, mo_data):
    # input
    data = [mo_data.top_level_node]
    operation = OperationType.CREATED
    status = Status.COMPLETE
...
@pytest.mark.skip(reason="Not implemented")
def test_mo_create_grouping_data(mo_main_updater, mo_data):
    # input
    data = [mo_data.grouping_node]
    operation = OperationType.CREATED
    status = Status.COMPLETE
    ...
    assert mo_nodes_count_before + 2 == mo_nodes_count_after
    assert mo_edges_count_before + 1 < mo_edges_count_after
    assert mo_path_edges_count_before + 1 == mo_path_edges_count_after
